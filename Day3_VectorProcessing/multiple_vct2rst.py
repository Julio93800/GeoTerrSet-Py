...
mask_path = "C:/Users/Admin/Desktop/Terrset/zones_raster_path"
...
for zone in os.listdir(mask_path):
    folder_name = zone.split(".")[0]

    # Define the path to the mask raster file
    if zone.endswith(".rst"):
        zone_path = os.path.join(mask_path, zone)
     
    
        output_path = os.path.join(output_dir, folder_name)
        
        # Load the mask raster
        mask_dataset = gdal.Open(zone_path, gdal.GA_ReadOnly)
        ...
        for root, dirs, files in os.walk(base_dir):
            for file in files:
                if pattern.match(file):  # Check if the filename matches the pattern
                    ...
                    output_rst = os.path.join(output_path, file.replace(".rst", "_pol.rst"))
...
print("\nAll raster operations completed. Results saved in:", output_path)
